...
import type { StatusLog, Profile, Occurrence, OrdemServico, AutoInfracao } from '@/lib/types';
import { STATUS_LABEL, ROLE_LABEL, STATUS_OS_LABEL, STATUS_OS_COLOR, ORIGEM_OS_LABEL, CIENCIA_LABEL, type UserRole, type OccurrenceStatus } from '@/lib/types';
...
export function AuditorPanel({ onOpenDetail }: Props) {
  const [tab, setTab] = useState<'trilha' | 'usuarios' | 'os'>('trilha');
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: osData, error: osErr } = await supabase.from('ordens_servico').select('*').order('criado_em', { ascending: false }).limit(100);
      if (!osErr) setOrdensServico((osData as OrdemServico[]) ?? []);
      const { data: aiData, error: aiErr } = await supabase.from('autos_infracao').select('*').order('criado_em', { ascending: false }).limit(100);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex gap-1 rounded-lg bg-muted p-1">
          {([['trilha','Trilha de auditoria'],['usuarios','Usuários'],['os','OS e Multas']] as const).map(([k, label]) => (
      ...
      {tab === 'os' && (
        <div className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base"><FileText className="h-4 w-4" />Ordens de Serviço</CardTitle>
              <CardDescription>Somente-leitura — auditoria de OS e autos de infração.</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base"><FileText className="h-4 w-4" />Autos de Infração</CardTitle>
              <CardDescription>Multas registradas durante vistorias.</CardDescription>
            </CardHeader>
            <CardContent>
              {autosInfracao.length === 0 ? (
                <EmptyState title="Nenhum auto de infração encontrado" />
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead><tr className="border-b text-left text-xs text-muted-foreground">
                      <th className="py-2">Valor</th><th>Autuado</th><th>Ciência</th><th>Motivo</th><th>Data</th>
                    </tr></thead>
                    <tbody>
                      {autosInfracao.map((ai) => (
                        <tr key={ai.id} className="border-b last:border-0">
                          <td className="py-2 font-bold text-primary">R$ {Number(ai.valor_multa).toFixed(2)}</td>
                          <td className="text-xs">{ai.autuado_nome ?? '—'}</td>
                          <td className="text-xs">{CIENCIA_LABEL[ai.ciencia_status]}</td>
                          <td className="text-xs max-w-[200px] truncate">{ai.motivo ?? '—'}</td>
                          <td className="text-xs">{new Date(ai.criado_em).toLocaleDateString('pt-BR')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
